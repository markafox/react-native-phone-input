...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    this.hasBlurred = this.hasBlurred.bind(this);
  ...
  countryData = PhoneNumber.getCountryDataByCode(this.props.initialCountry);
  state = {
    iso2: this.props.initialCountry,
    disabled: this.props.disabled,
    formattedNumber: this.countryData ? `+${this.countryData.dialCode}` : "",
  ...
  onChangePhoneNumber(number) {
    this.updateFlagAndFormatNumber(number, () => {
      ...
      if (this.props.onChangePhoneNumber) {
        this.props.onChangePhoneNumber(number, isValid, this.state.hasBlurred);
      }
    });
  }
  ...
  hasBlurred() {
    const isValid = this.isValidNumber();
    this.setState({ hasBlurred: true, isValid });
    if (this.props.onChangePhoneNumber) {
      this.props.onChangePhoneNumber(this.state.inputValue, isValid, true);
    }
}
  getCountryCode() {
  ...
  isValidNumber() {
    if (!this.state.inputValue || this.state.inputValue.length < 3) return false;
